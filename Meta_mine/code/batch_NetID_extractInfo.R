...
for(i in 1:length(foldernames_batch_CPLEX)){
  ...
  if(any(list.files() == "mdata.csv") & any(list.files() == "optimized.RData")){
...
print(batch_CPLEX)
select_batch_CPLEX_test = batch_CPLEX
print(select_batch_CPLEX_test)


for(extract_i in 1:length(select_batch_CPLEX_test)){
  # for(i in 3:4){
  main_dir = getwd()
  while(basename(main_dir)!="Meta_mine"){
  ...
  setwd(select_batch_CPLEX_test[extract_i])
  print(select_batch_CPLEX_test[extract_i])
  rm(list = "CPLEX_all_x")
  load("optimized.RData")
  
  if(!exists("CPLEX_all_x")){next}
  ...
  determine_is_metabolite = function(){
    ...
    # edge_info_sum2 = CPLEXset$data$edge_info_sum
    ...
    relation_list["linktype"] = EdgeSet$Merge$linktype[relation_list$edge_id]
  ...
  # Graphic analysis ####
  {
    ...
    # write.csv(EdgeSet$, "heterodimer.txt", row.names = F)
  }
  ...
  save(Mset, EdgeSet, CPLEXset, file ="optimized.RData")
