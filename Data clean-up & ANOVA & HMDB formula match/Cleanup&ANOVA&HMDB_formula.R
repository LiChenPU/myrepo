...
#setwd("C:/Users/Li Chen/Desktop/Henry")
setwd("C:/Users/Li Chen/Dropbox/HRMass_ID/180629 Triwizard Tournament/Data clean-up & ANOVA & HMDB formula match")
...
mode = -1
normalized_to_col_median = FALSE
...
#Data manipulation to increase replicate number
{
  ...
  if(data_replicate_number <3)
  {
    for (i in names(table(sample_cohort))){
      raw[paste(i,"_mean",sep="")]=rowMeans(raw[, 14+which(sample_cohort==i)])
    }
  }
  sample_names=colnames(raw)[15:ncol(raw)]
  if(length(grep("blank|blk", sample_names, ignore.case = T))!=0){
...
s6[,4:(ncol(s6)-2)]=s6[,4:(ncol(s6)-2)]/rowMeans(s6[,4:(ncol(s6)-2)])
col_median = data.frame(col_median=apply(s6[,4:(ncol(s6)-2)],2,median))
 
col_median
write.csv(col_median, "col-median.csv", row.names = T)
}
