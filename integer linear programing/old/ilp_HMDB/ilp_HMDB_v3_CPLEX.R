...
if(!read_from_csv)
{
  ...
  #for(n in 1:2000){
    ...
    for(temp_formula in unique(formula_1$formula)){
      ...
      if(temp_formula_2 %in% formula_2$formula){
        temp_j1 = which(unknown_formula$formula==temp_formula&unknown_formula$id==node_1)
        temp_j2 = which(unknown_formula$formula==temp_formula_2&unknown_formula$id==node_2)
        
        
        

        #if one node is library node,
        if(node_1 %in% lib_nodes$ID){
        ...
        if((node_1 %in% unknown_nodes$ID) & (node_2 %in% unknown_nodes$ID)){
          triplet_edge_ls_edge[[temp_i]] = data.frame(i=temp_i,
                                            j=temp_j,
                                            v=2)
  ...
  write.csv(edge_info_sum,"edge_info_sum.csv",row.names=F)
...
#Other parameters
{
  ...
  dir <- c(rep("==",nrow(unknown_nodes)), rep("<=", nrow(edge_info_sum)))
...
#                             control = list(tm_limit=1*60*1000,verbose=T))
...
}
{
  ...
  CPLEXresult=solutionCPLEX(env, prob)
  unknown_formula["CPLEX_result"] = CPLEXresult$x[1:nrow(unknown_formula)]
  test=merge(raw_node_list, unknown_formula[unknown_formula$CPLEX_result==1,], by.x = "ID",by.y="id")
  pred_dif=test[test$MF!=test$Predict_formula,]
  pred_dif=test[test$MF!=test$formula,]
