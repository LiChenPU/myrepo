...
#Clean up
{
  ...
  # 
  # #pred_dif = node_list_pred[node_list_pred$MF!=node_list_pred$formula,]
  # 
...
if(!read_from_csv)
{
  ...
  triplet_edge_ls_edge_sum$j=triplet_edge_ls_edge_sum$j+nrow(merge_formula)
...
#Other parameters
{
  ...
  obj <- c(rep(0,nrow(lib_formula)), rep(0, nrow(unknown_formula)), edge_info_sum$edge_score)
...
unknown_formula["ILP_result"] = para$ILP_result.solution[(nrow(lib_formula)+1):(nrow(lib_formula)+nrow(unknown_formula))]
edge_info_sum["ILP_result"] = para$ILP_result.solution[(nrow(lib_formula)+nrow(unknown_formula)+1):nrow(para)]
