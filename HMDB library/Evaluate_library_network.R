...
edge_list_origin = edge_list
edge_list = edge_list[edge_list$edge_massdif_score>0.98,]
...
pred_same = merge_node_list_pred[merge_node_list_pred$MF==merge_node_list_pred$Predict_formula,]

# illegal_formula = merge_node_list[grepl("Illegal_formula",merge_node_list$Predict_formula),]
}


#nodes in network, what are they?
{
  network_all_category = hmdb_merge[hmdb_merge$Formula %in% merge_node_list$MF & !is.na(hmdb_merge$Status),]
  
  hmdb_quantified = network_all_category[network_all_category$Status=="quantified",]
  hmdb_detected = network_all_category[network_all_category$Status=="detected",]
  hmdb_expected = network_all_category[network_all_category$Status=="expected",]
  hmdb_predicted = network_all_category[network_all_category$Status=="predicted",]
  
  formula_quantified = hmdb_quantified[!duplicated(hmdb_quantified$MF),]
  formula_detected = hmdb_detected[!duplicated(hmdb_detected$MF)&(!hmdb_detected$MF %in% formula_quantified$MF),]
  formula_expected = hmdb_expected[!duplicated(hmdb_expected$MF)&
                                     (!hmdb_expected$MF %in% formula_detected$MF)&
                                     (!hmdb_expected$MF %in% formula_quantified$MF),]
  formula_predicted = hmdb_predicted[!duplicated(hmdb_predicted$MF)&
                                       (!formula_predicted$MF %in% formula_detected$MF)&
                                       (!formula_predicted$MF %in% formula_quantified$MF)&
                                       (!formula_predicted$MF %in% formula_expected$MF),]
  
  formula_quantified_network = formula_quantified[formula_quantified$MF %in% pred_same$MF,]
  formula_detected_network = formula_detected[formula_detected$MF %in% pred_same$MF,]
  formula_expected_network = formula_expected[!formula_expected$MF %in% pred_same$MF,]
  #write.csv(formula_quantified_network,"quantified_not_in_network.csv",row.names = F)
  #write.csv(formula_detected_network,"detected_not_in_network.csv",row.names = F)
  #write.csv(formula_expected_network,"expected_not_in_network.csv",row.names = F)
}
...
#Difference between SDF file and XML files
# test2 = test[test$MF!=test$Formula,]
# test2 = test2[(!is.na(test2$MF)),]
# test3 = test2[test2$Charge==0,]
# write.csv(test3, "inconsistent SDF&XML.csv")
# test4 = test3[,c("HMDB_ID","MF","Formula")]
