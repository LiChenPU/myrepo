...
#Read data
{
  ...
  hmdb_endogenous = read_csv("HMDB_Extracted_0717.csv")
  merge_library = merge(data_known, hmdb_endogenous, by.x = "MF", by.y="Formula", all=T)
  merge_library = merge_library[!is.na(merge_library$HMDB_ID),]
}
...
te.csv(merge_node_list[merge_node_list$category==1|merge_node_list$category==2,], "predict_pos.csv")
