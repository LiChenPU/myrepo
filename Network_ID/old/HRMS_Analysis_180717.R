...
setwd("C:/Users/Li Chen/Dropbox/HRMass_ID/180629 Triwizard Tournament/Network_ID")
...
node_list = raw[,c("ID", "mz","RT","MF", "category","Name")]
...
HMDB_node_list[,1]=HMDB_node_list[,1]+nrow(node_list)
...
#calculate mass difference matrix
{
  ...
  ms_dif_matrix = matrix(data=as.numeric(),nrow=nrow(node_list), ncol=nrow(node_list))
  for(i in 1:merge_nrow){
    ms_temp = rep(merge_node_list$mz[i], merge_nrow)
    ms_dif_matrix[,i]=class(ms_temp-merge_node_list$mz)
  }
}
...
# write.csv(edge_list_sub, "edge_list_sub.csv",row.names = F)
...
output_network_csv = merge_node_list[vertex.attributes(g_intrest)$name,]
