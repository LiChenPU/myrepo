...
## View Results ####
{
  ...
  test3_filter = test3 %>%
    ...
    # filter(category == "Fragment") %>%
    filter(Feature == "Metabolite") %>%
    # filter(!Feature %in% c("Metabolite", "TimeSS"), !is.na(Feature)) %>%
  ...
  crosstable = tabyl(test3, Feature, Feature...23)
...
## Query specific nodes or edges ####
{
  ...
  ilp_edges_node_related = ilp_edges %>%
    ...
    # filter(ilp_nodes1 %in% c(34808,34810) | ilp_nodes2 %in% c(34808,34810)) %>%
